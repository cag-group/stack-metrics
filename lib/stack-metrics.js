...
class StackMetrics {
   ...
   * @param sendInterval (optional) Interval to send values to Stackdriver Monitoring API, milliseconds. If not bigger than 0, no automatic
   * sending are performed and sending is done by explicitly calling sendValues(timestamp)
   * @param keyFilename (optional) Filename for service account key. Not required when running in Google Cloud
   */
  constructor (projectId, appName, sendInterval, keyFilename) {
    if (projectId === undefined) {
      throw new Error(`Invalid arguments: 'projectId' cannot be undefined`)
    }
    if (appName === undefined) {
      throw new Error(`Invalid arguments: 'appName' cannot be undefined`)
    }
    ...
    if (keyFilename) {
      fs.stat(keyFilename, (err, stats) => {
        this._init(sendInterval, !err && stats.isFile() ? keyFilename : undefined)
      })
    } else {
      this._init(sendInterval)
    }
  }

  _init (sendInterval, keyFilename) {
    ...
    if (sendInterval > 0) {
      ...
      logger.warn(`Automatic send interval not started. Parameter 'sendInterval' must be bigger than 0, was ${sendInterval}`)
    }
  }
   ...
   * @param getValue Optional async function for reading metric value as an alternative to setting it later. Called just before sending values to stackdriver
   * @returns {StackMetric} Request object
   */
  createMetric (metricName, metricDescription, metricType, getValue) {
  ...
  async sendValues (timestamp) {
    ...
    await Promise.all(Array.from(this.metricsMap.values())
      .filter(metric => metric.getValue !== undefined)
      .map(metric => {
        return Promise.resolve(metric.getValue(metric))
          .then(value => {
            metric.value = value
          })
      }))
  ...
  async _sendTimeSeries (timestamp) {
    const timeSeriesData = Array.from(this.metricsMap.values())
      .filter(metric => metric.type === StackMetrics.TYPE_RATE || metric.value !== undefined) // Filter unwritten non-rate metrics
