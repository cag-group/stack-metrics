...
class StackMetrics {
  ...
  constructor (keyFilename, projectId, appName, envName, metricGroupName, sendInterval) {
    ...
    if (sendInterval !== 0) {
      setInterval(() => {
        try {
          this.sendValues(Date.now)
        } catch (err) {
          logger.warn('sendValues failed:', err)
        }
      }, sendInterval)
    }
  ...
  static _getMetricName (type) {
    // Example type: 'custom.googleapis.com/testapp/testValue1'
  ...
  async _sendTimeSeries (timestamp) {
    ...
    const timeSeriesData = Array.from(this.metricsMap.values())
      .filter(metric => metric.rateInterval || metric.value)
